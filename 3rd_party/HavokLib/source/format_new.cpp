...
HWND GetMaxMainWindow() {
  // 2 attempts at getting max's window handle, 3rd is a fallback
  HWND hwnd = FindWindowA("3DSMAX", NULL);
  if (hwnd) return hwnd;
  hwnd = FindWindowA(NULL, "Autodesk 3ds Max");
  if (hwnd) return hwnd;
...
std::string OpenFileDialog(const std::string& initialDir) {
	...
	ofn.hwndOwner = GetMaxMainWindow();
	ofn.lpstrFile = filename;
	ofn.nMaxFile = MAX_PATH;
	ofn.lpstrFilter = "Compendium Files (*.compendium)\0*.compendium\0All Files (*.*)\0*.*\0";
	ofn.nFilterIndex = 1;
	ofn.lpstrInitialDir = initialDir.c_str();
	ofn.lpstrTitle = "Select Havok Compendium File";
...
std::string FindCompendiumWindows(const std::string& sourceFilePath) {
	// Extract directory from source file path
	size_t lastSlash = sourceFilePath.find_last_of("\\/");
	std::string directory = sourceFilePath.substr(0, lastSlash + 1);
	std::string searchPattern = directory + "*.compendium";
	...
	FindClose(hFind);
	printwarning("[Havok] No compendium file found in " << directory);
	printline("[Havok] Select compendium file manually...");

	std::string selectedPath = OpenFileDialog(directory);
	if (!selectedPath.empty()) {
		return selectedPath;
	}
	throw std::runtime_error("No compendium file found");
}
...
template <> struct Read<CompileFourCC("TCRF")> {
	static auto e(BinReaderRef rd, hkChunk *holder, hkxNewHeader *root) {
		...
		try {
			std::string compendiumPath = FindCompendiumWindows(root->sourceFilePath);

			BinReader tcmReader(compendiumPath);

			hkChunk tcmHeader;
			tcmReader.Read(tcmHeader);
			tcmHeader.Reorder();

			if (tcmHeader.tag != CompileFourCC("TCM0")) {
				throw es::InvalidHeaderError(tcmHeader.tag);
			}

			const uint32 compendiumSize = tcmHeader.Size();
			const size_t endPos = tcmReader.Tell() + compendiumSize - 8;

			// Process compendium chunks, populating the tagfile root header
			while (tcmReader.Tell() < endPos) {
				hkChunk compChunk;
				compChunk.Read(tcmReader, root); // Pass the SAME root!
			}

		}
		catch (const std::exception& e) {
			printwarning("[Havok] Failed to load compendium: " << e.what());
		}
	};
};
...
template <> struct Read<CompileFourCC("TCID")> {
	static auto e(BinReaderRef rd, hkChunk *holder, hkxNewHeader *root) {
		...
		std::vector<uint64> tcmHashes;

		const uint32 numHashes = holder->Size() / sizeof(uint64);

		rd.ReadContainer(tcmHashes, numHashes);

		bool foundMatch = false;
		for (const auto& hash : tcmHashes) {
			if (hash == root->tcmHash) {
				foundMatch = true;
				break;
			}
		}
		...
		if (!foundMatch) {
			throw es::InvalidHeaderError(holder->tag);
		}
...
template <> struct Read<CompileFourCC("TST1")> : Read<CompileFourCC("TSTR")> {};
...
template <> struct Read<CompileFourCC("FST1")> : Read<CompileFourCC("FSTR")> {};
...
static const std::map<uint32, decltype(&Read<0>::e)> hkChunkRegistry = {
	...
	makeSkip<CompileFourCC("TBDY")>(), // TODO
	...
	make<CompileFourCC("TST1")>(),     //    
	make<CompileFourCC("FST1")>(),     //    
	make<CompileFourCC("TCRF")>(),     //  *.hkx Type compendium hash
	make<CompileFourCC("TCID")>(),     //  *.compendium hashes
};
